...
def test_request_uniprot_file():
    ...
    #Invalid uniprot_id
    assert request_uniprot_file("ImNotAnId", "xml") == None

    #Invalid filetype
    assert request_uniprot_file("p02070", "ImNotAFiletype") == None

    #Non-string uniprot_id
    assert request_uniprot_file(2070, "xml") == None
...
def test_parse_uniprot_xml():
    ...
    #Message to let user know tests are finished without error
    print("parse_uniprot_xml: all tests finished without error")

def test_uniprot_get_entries():
    #Valid A
    test = uniprot_get_entries("O79680")
    comparison = 'O79680'
    assert test[0].entry_data["id"] == comparison
...
def test_uniprot():
    test_request_uniprot_file()
    test_parse_uniprot_xml()
    test_uniprot_get_entries()
    print("all tests finished without error!")
    return
